...
3. **Очистка общая:** `make clean` и `make clean-all` затрагивают весь Docker на хосте, а не только админку. Можно запускать из любой директории.
4. **Не выполнять `docker system prune -af`** при обычном деплое — это убивает всё. Использовать только по явной просьбе.
5. **Для билда — `--no-cache`:** чтобы собирать с нуля и не тащить старый кеш.
6. **Перед билдом:** убедиться, что код уже обновлён (`git pull` или rsync).
